...
class Study:
    ...
    def __init__(self, session_def: session.SessionDefinition, planes: list[plane.Definition], planes_per_interval: dict[episode.Event,list[str]], working_directory: str|pathlib.Path):
    ...
    def store_as_json(self, path: str | pathlib.Path):
        ...
        with open(d_path, 'w') as f:
            to_dump = {k:getattr(self,k) for k in ['planes_per_interval']}    # only this field. Name will be populated from name of session/provided folder, recordings from each subfolder in the session/provided folder, and working_directory as the provided path
            to_dump['planes_per_interval'] = [(k, to_dump['planes_per_interval'][k]) for k in to_dump['planes_per_interval']]
            # dump to file
    ...
    def load_from_json(path: str | pathlib.Path) -> 'Study':
        ...
        kwds['planes_per_interval'] = {k:v for k,v in kwds['planes_per_interval']}
        # get session def
