...
class ProjectFilter(DefaultFilter):
    def __init__(self, extensions={'.json', '.gazeMapper'}, do_report_directories:bool=False, exclude_paths:set[pathlib.Path]=None, files_only_modified=False, dirs_only_added_deleted=False):
        ...
        self.files_only_modified    = files_only_modified
        self.dirs_only_added_deleted= dirs_only_added_deleted
    ...
    def __call__(self, change: Change, path: str) -> bool:
        ...
        if change==Change.deleted:
            ...
            if path.is_dir():
                ...
                return self.do_report_directories and (not self.dirs_only_added_deleted or change==Change.added)
            elif self.files_only_modified and change!=Change.modified:
                return False
            else:
                return path.suffix in self.extensions
