...
|Video export: Show gaze projected to plane?|`video_show_gaze_on_plane`|`True`|If `True`, gaze projected to the plane (both the gaze point on the video, and the left and right eye\'s gaze vectors if available) is drawn on the video for eye tracker recordings.|
